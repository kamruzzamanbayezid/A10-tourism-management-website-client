...
const TouristSpot = ({ touristSpot }) => {
      ...
      const { image, tourists_spot_name, country_Name, location, short_description, average_cost, seasonality, travel_time, totalVisitorsPerYear, userEmail, userName } = touristSpot;
            ...
            <div className="bg-white card-style p-5 hover-card mb-10 flex flex-col">
                  <div className='relative'>
                        ...
                        <div id='view-btn' className='hidden absolute bg-[#00095E] text-white inset-16 items-center justify-center text-lg font-medium'>
                              <Link ><button className='hover:underline'>View Details</button></Link>
                        </div>
                        <div className='absolute -bottom-7 left-5 bg-red-500 border-radius-style button-slanted'>
                              <span className='button-slanted-content flex items-center gap-2 text-white font-normal'> <IoMdTime className='text-xl text-white' />{travel_time}</span>
                        </div>
                  </div>
                  <h2 className='text-2xl font-bold mt-8 mb-2 text-blue flex-1 font-josefin'>{tourists_spot_name}</h2>
                  ...
                  <span className='text-blue font-medium '>#{country_Name}</span>
                  <hr className='my-4' />
                  <div className='grid grid-cols-2 gap-5'>
                        <span className='text-sm flex items-center gap-2'><IoIosPricetags className='text-xl text-yellow' /> ${average_cost}</span>
